describe("Cart and Checkout Test", ()=>{
    ...
    it("Add product to cart and verify cart contents", ()=>{
        cy.get("#tbodyid .card").should("be.visible").and("have.length.at.least", 1).its("length").then((length)=>{
            const randomIndex = Math.floor(Math.random() * length);
            // Add the first product to cart
            cy.get("#tbodyid .card").eq(randomIndex).within(()=>{
            ...
            cy.get(`[onclick='addToCart(${randomIndex + 1})']`).should("be.visible").and("have.css", "background-color", "rgb(52, 168, 83)").and("contain", "Add to cart").click({force:true});
            cy.wait(1000); // Wait for the alert to be handled
            // Go to homepage and add a second product to cart
            cy.get(".nav-item [href='index.html'] ").should("be.visible").click({force:true});
            cy.get("#tbodyid .card").should("be.visible").and("have.length.at.least", 1).its("length").then((length)=>{
            // Add second product to cart
            const secondRandomIndex = Math.floor(Math.random() * length);   
            cy.get("#tbodyid .card").eq(secondRandomIndex).within(()=>{
                cy.get(".card-title a").click({force:true});
            });
            cy.url().should('include', 'prod.html');
            cy.wait(1000); // Wait for product details to load
            ...
            cy.get(`[onclick='addToCart(${secondRandomIndex + 1})']`).should("be.visible").and("have.css", "background-color", "rgb(52, 168, 83)").and("contain", "Add to cart").click({force:true});
            cy.wait(1000); // Wait for the alert to be handled
        ...
        cy.get("#page-wrapper h2").should("be.visible").and("include.text", "Products");
        cy.get("#tbodyid tr").should("have.length", 2).each(($el)=>{
            cy.wrap($el).within(()=>{
    ...
    it("Complete checkout process", ()=>{
        ...
        cy.get('#totalp').should("be.visible").and("not.be.empty");
        // Place order
        cy.get("#page-wrapper .btn-success").should("be.visible").and("contain", "Place Order").and("have.css", "background-color", "rgb(52, 168, 83)").click({force:true});
        cy.get("#orderModal .modal-content").should("be.visible").and("contain", "Place order").within(()=>{
            ...
            cy.get("#card").should("be.visible").and("be.empty").type("1234 5678 9012 3456");
            cy.get("#month").should("be.visible").and("be.empty").type("12");
            cy.get("#year").should("be.visible").and("be.empty").type("2025");
            cy.get(".btn-primary").scrollIntoView().should("be.visible")
                .and("have.css", "background-color", "rgb(2, 117, 216)")
                .and("have.text", "Purchase").click({force:true});
        });
        cy.get(".sweet-alert").should("be.visible").and("contain", "Thank you for your purchase!").within(()=>{
            ...
            cy.get(".confirm").should("be.visible").and("have.css", "background-color", "rgb(2, 117, 216)").and("contain", "OK").click({force:true});
        });
